...
import org.mswsplex.basic.utils.Cuboid;
...
public class FillCommand implements CommandExecutor, TabCompleter {
	...
	public FillCommand(Main plugin) {
		...
		cmd.setPermissionMessage(MSG.getString("NoPermission", "&cNo Permission"));
	}
...
//					args[5].equals("~") ? p.getZ() : Double.parseDouble(args[5]));
		...
		if (amo >= plugin.getConfig().getInt("Max.Fill.Confirm") && (sender instanceof Player)
				&& !manager.getBoolean(((Player) sender), "confirmed")) {
			...
			for (String res : args)
				all = all + res + " ";
			manager.setInfo((Player) sender, "confirmCommand", all);
			return true;
		}
