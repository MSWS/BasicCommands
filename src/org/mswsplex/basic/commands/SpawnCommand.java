...
public class SpawnCommand implements CommandExecutor, TabCompleter {
	public SpawnCommand() {
		...
		Main.plugin.getCommand("spawn").setTabCompleter(this);
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		case "item":
			...
			MSG.tell(sender, MSG.getString("Command.Spawn.Spawn", "You spawned %amo% %type%")
					.replace("%prefix%", MSG.getString("Command.Spawn.Prefix", "Spawn")).replace("%amo%", amo + "")
		...
		default:
			try {
				...
				try {
					...
					player.getWorld().dropItem(player.getLocation(), new ItemStack(type, amo));
					MSG.tell(sender,
							MSG.getString("Command.Spawn.Spawn", "You spawned %amo% %type%")
									.replace("%prefix%", MSG.getString("Command.Spawn.Prefix", "Spawn"))
									.replace("%amo%", amo + "").replace("%type%", MSG.camelCase(type.toString()))
									.replace("%s%", amo == 1 ? "" : "s"));
					return true;
				} catch (Exception ee) {
					MSG.tell(sender, MSG.getString("Command.Spawn.Unknown", "unknown entity type").replace("%prefix%",
							MSG.getString("Command.Spawn.Prefix", "Spawn")));
					return true;
				}
			}
			break;
		...
		int nearby = player.getNearbyEntities(20, 20, 20).size(), max = Main.plugin.config.getInt("MaxSpawnLimit");
		if (max != -1) {
			if (amo + nearby > max) {
				amo = max - nearby;
				MSG.tell(sender,
						MSG.getString("Command.Spawn.MetMax", "spawning %amo% instead")
								.replace("%prefix%", MSG.getString("Command.Spawn.Prefix", "Spawn"))
								.replace("%amo%", amo + ""));
			}
		}
